...
- **Services**: Imports core server modules (`modules/server/default.nix`) enabling declarative Samba storage sharing, Jellyfin media streaming server, Nextcloud cloud hub, Transmission BitTorrent daemon, OpenVPN tunneling, local DNS resolution, Nginx reverse proxy, and the Homepage portal.
...
- **Nextcloud Hub** (`nextcloud.nix`): Self-hosted cloud storage, files, and collaboration platform served at `nextcloud.jeff.lan` backed by local PostgreSQL and Redis caching.
...
- **Reverse Proxy, DNS & HTTPS** (`reverse-proxy.nix`, `dns.nix`): Local Nginx reverse proxy enforcing HTTPS (`forceSSL`) with automatic HTTP-to-HTTPS redirection across all `*.jeff.lan` services (`jeff.lan`, `nextcloud.jeff.lan`, `jellyfin.jeff.lan`, `transmission.jeff.lan`). Wildcard SSL certificates and local Root CA are managed automatically and stored on the Partage drive (`/mnt/partage/ssl`).
