...
- **RomM** (`romm.nix`): Retro game ROM manager and player powered by OCI containers (Podman), MariaDB, and game libraries at `/mnt/exp6/EXP6/ROMs`.
- **Reverse Proxy, DNS & HTTPS** (`reverse-proxy.nix`, `dns.nix`): Local Nginx reverse proxy enforcing HTTPS (`forceSSL`) with automatic HTTP-to-HTTPS redirection across all `*.jeff.lan` services (`jeff.lan`, `jellyfin.jeff.lan`, `transmission.jeff.lan`, `romm.jeff.lan`). Wildcard SSL certificates and local Root CA are managed automatically and stored on the Partage drive (`/mnt/partage/ssl`).
