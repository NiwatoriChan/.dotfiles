...

{
  ...
  # Enable XDG Desktop Portals (required by Flatpak and DE integrations)
  xdg.portal = {
    enable = true;
    extraPortals = [
  ...
  # Register the session desktop entry at the system level for SDDM
  services.displayManager.sessionPackages = [
    ((pkgs.writeTextDir "share/wayland-sessions/KineticWE.desktop" ''
      [Desktop Entry]
      Name=KineticWE
      Comment=A Kwin Window Environment
      Type=Application
      Exec=${nixosArgs.config.programs.kineticwe.package}/bin/start-kineticwe
      DesktopNames=KDE
    '').overrideAttrs (old: {
      passthru = (old.passthru or {}) // {
        providedSessions = [ "KineticWE" ];
      };
    }))
  ];
  ...
  home-manager.users.niwatorichan = { config, ... }: {
    # Custom start-kineticwe package
    home.packages = [
      nixosArgs.config.programs.kineticwe.package
    ];

    # Register the session desktop entry
